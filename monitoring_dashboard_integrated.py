...
class MonitoringDashboard:
    ...
    def _classify_error(self, error: Exception, context: str = "") -> Dict[str, Any]:
        """
        Classify errors into structured taxonomy with user-friendly messages
        Returns: {"category": str, "severity": str, "message": str, "action": str}
        """
        ...
        if "connection" in error_str or "connect" in error_str or "network" in error_str:
            if "timeout" in error_str or "timed out" in error_str:
                return {
                    ...
                    "severity": "warning",
                    "message": "⏱️ Connection timed out. The server might be busy.",
                    "action": "Try again in a few moments or check server status."
                }
                ...
                return {
                    ...
                    "action": "Verify network connectivity and server availability."
                }
            else:
                return {
        ...
        # Authentication/Authorization Errors
            ...
            return {
                ...
                "severity": "error",
                "message": "🔐 Authentication failed. Please reconnect to the API.",
                "action": "Click 'Connect to API' to refresh your authentication."
            }
        ...
        # Server Errors
            ...
            return {
                ...
                "action": "The issue has been reported. Please try again later."
            }
            ...
            return {
                ...
                "severity": "warning",
                "message": "🏗️ Service is temporarily down for maintenance.",
                "action": "Please try again later."
            }

        # Validation Errors
        elif "422" in error_str or "validation" in error_str or "invalid" in error_str:
            return {
                "category": "VALIDATION_ERROR",
                "severity": "warning",
                "message": "📝 Task validation failed.",
                "action": "Check the task description and try rephrasing it."
            }

        # Task-specific Errors
        elif "confidence" in error_str and "threshold" in error_str:
            return {
                "category": "CONFIDENCE_THRESHOLD",
                "severity": "warning",
                "message": "🎯 Task confidence too low for automatic assignment.",
                "action": "Try providing more specific task details or choose a different agent."
            }
        ...
        # Rate Limiting
            ...
            return {
                ...
                "action": "Wait a moment before submitting another task."
            }
        ...
        # Timeout Errors
            ...
            return {
                ...
                "severity": "warning",
                "message": "⏱️ Request took too long to complete.",
                "action": "The task might still be processing. Check 'Pending Tasks' below."
            }
    ...
    def _display_error(self, error: Exception, context: str = ""):
        ...
        if error_info["severity"] == "error":
            ...
            st.warning(f"{error_info['message']}\n\n💡 {error_info['action']}")
        else:
            st.info(f"{error_info['message']}\n\n💡 {error_info['action']}")

        # Log for debugging
        logger.warning(f"Error classified as {error_info['category']}: {str(error)}")
    ...
    def _validate_request_payload(self, payload: Dict[str, Any], schema_class) -> Dict[str, Any]:
        ...
        try:
            ...
            self._display_error(e, "request validation")
    ...
    def _emit_telemetry(self, event_type: str, data: Dict[str, Any]):
        ...
        try:
            ...
            # Send to telemetry endpoint (could be InfluxDB, Prometheus, etc.)
            telemetry_url = "http://localhost:8086/write?db=agent_lightning"  # InfluxDB example

            # For now, just log the telemetry data
            logger.info(f"TELEMETRY: {event_type} - {telemetry_data}")

            # In production, this would send to actual telemetry system
            # response = requests.post(telemetry_url, data=json.dumps(telemetry_data))
    ...
    def _check_sla_compliance(self, priority: str) -> Dict[str, Any]:
        ...
        # Emit SLA compliance telemetry
        self._emit_telemetry("sla_check", {
            "priority": priority,
            "compliant": compliant,
    ...
    def _make_resilient_request(self, url: str, method: str = "GET",
        ...
        for attempt in range(max_retries):
            try:
                ...
                logger.warning(f"Connection error to {url} (attempt {attempt + 1}/{max_retries})")
            except Exception as e:
    ...
    def _render_task_assignment(self):
        ...
        with col2:
            if st.button("🔌 Connect to API"):
                try:
                    # Authenticate with Enhanced API using resilient request
                    response = self._make_resilient_request(
                        "http://localhost:8002/api/v1/auth/token?username=admin&password=admin",
                        method="POST",
                        timeout=5
                    )
                    if response and response.status_code == 200:
                        ...
                        # Fetch available agents from enhanced API using resilient request
                        try:
                            agents_response = self._make_resilient_request(
                                "http://localhost:8002/api/v2/agents/list",
                                method="GET",
                                timeout=5
                            )
                            if agents_response and agents_response.status_code == 200:
                        ...
                        self._display_error(Exception(error_msg), "API connection")
                except Exception as e:
                    self._display_error(e, "API connection")
        
        if st.session_state.api_connected:
            st.success("✅ API Connected - Ready to assign tasks")

            # SLA Health Check Banner
            sla_status = self._check_sla_compliance("high")  # Check against high priority requirements
            if not sla_status["compliant"]:
                st.error(f"⚠️ **SLA Alert**: System health degraded. Unhealthy services: {', '.join(sla_status['unhealthy_services'])}. High-priority tasks may be rejected.")
                if sla_status["warnings"]:
                    for warning in sla_status["warnings"]:
            ...
            with col1:
                ...
                # AI Model Selection - Get available models dynamically
                try:
                    from agentlightning.llm_providers import list_supported_models
                    ...
                    logger.warning(f"Could not load dynamic model list: {e}")
                ...
                ai_model = st.selectbox(
                    ...
                    help="Choose the AI model for task execution (OpenAI, Anthropic, or Grok)"
                )
                    ...
                    try:
                        ...
                        if priority in ["urgent", "high"]:
                            ...
                            if not sla_check["compliant"]:
                                ...
                                # Emit telemetry for SLA rejection
                                self._emit_telemetry("task_rejected_sla", {
                                    "priority": priority,
                                    "unhealthy_services": sla_check["unhealthy_services"],
                                    "warnings": sla_check["warnings"],
                        ...
                        request_payload = {
                            ...
                            "deployment_config": deployment_config if deployment_type != "none" else None
                        }
                        ...
                        self._emit_telemetry("task_validation", {
                            ...
                            "agent_type": agent_type,
                            "ai_model": ai_model,
                            "has_deployment": deployment_type != "none",
                        ...
                        if task_response.status_code == 200:
                                ...
                                if status_response.status_code == 200:
                                    ...
                                    if task_status.get("status") in ["completed", "failed"]:
                                        ...
                                        if ai_response:
                                            ...
                                            # Add feedback buttons for Q-learning improvement
                                            st.markdown("---")
                                            col_feedback1, col_feedback2, col_feedback3 = st.columns([1, 1, 2])

                                            with col_feedback1:
                                                if st.button("👍 Good", key=f"good_{task_id}"):
                                                    try:
                                                        ...
                                                        self._display_error(e, "feedback submission")

                                            with col_feedback2:
                                                if st.button("👎 Poor", key=f"poor_{task_id}"):
                                                    try:
                                                        # Validate feedback request
                                                        feedback_payload = {
                                                            "task_id": task_id,
                                            ...
                                            with col_feedback3:
                                                st.caption("Help improve the AI by rating this response")
                            ...
                            # Try to classify the error from the response
                            try:
                                error_data = task_response.json()
                                if "error" in error_data:
                                    # This is a structured error from the API
                                    error_info = self._classify_error(Exception(error_data.get("message", task_response.text)), "task submission")
                                    self._display_error(Exception(error_data.get("message", task_response.text)), "task submission")
                                else:
                                    self._display_error(Exception(f"HTTP {task_response.status_code}: {task_response.text}"), "task submission")
                            except:
                                self._display_error(Exception(f"HTTP {task_response.status_code}: {task_response.text}"), "task submission")
                        ...
                        self._display_error(e, "task submission")
                    ...
                    with col2:
                        if st.button(f"Check", key=f"check_{pending_task['task_id']}"):
                            try:
                                ...
                                status_response = self._make_resilient_request(
                                ...
                                self._display_error(e, "task status check")
                ...
                with st.spinner("Agent thinking..."):
                    try:
                        ...
                        if response.status_code == 200:
                            ...
                            self._display_error(Exception(f"Chat failed: {response.text}"), "chat")

                    except Exception as e:
                        self._display_error(e, "chat")
