...
class EnhancedAgentService:
    ...
    async def process_task_with_knowledge(self, request: AgentRequest, execute_code: bool = False) -> TaskResponse:
        ...
        try:
            # FORCE ACTUAL EXECUTION for analysis tasks
            needs_file_access = any(keyword in request.task.lower() for keyword in [
                'analyze', 'project', 'directory', 'files', 'code', 'ios', 'android', 'examine', 'status', 'report'
            ])
            
            # Handle directory access with authorization
            if needs_file_access and ('/' in request.task or 'directory' in request.task.lower()):
            ...
            if needs_internet:
                try:
                    ...
                    if api_key:
                        ...
                        completion = client.chat.completions.create(
                            model=agent.model if agent else "gpt-4o",
                            messages=[
                ...
                # Regular OpenAI API for non-internet tasks
                api_key = os.getenv("OPENAI_API_KEY")
                if api_key:
                    client = openai.OpenAI(api_key=api_key)
                    ...
                    full_system_prompt = system_prompt
                    if deployment_config:
                        full_system_prompt += "\n\nYou have code deployment capabilities."
