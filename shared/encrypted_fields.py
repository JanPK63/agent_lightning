...
class EncryptedField(TypeDecorator):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.key_id = key_id
        self.plaintext_type = (
            plaintext_type(length=length, **kwargs)
            if length
            else plaintext_type(**kwargs)
        )
        super().__init__()
    ...
    def process_bind_param(self, value: Any, dialect: Dialect) -> Optional[bytes]:
        ...
        try:
            ...
            encrypted_data = _get_key_management_service()._encrypt_data(
                value.encode("utf-8"), encryption_key
            )
            ...
            if isinstance(self.plaintext_type, String):
                ...
                try:
                    ...
                    if self.plaintext_type.python_type == int:
                        ...
                        decrypted_value = decrypted_value.lower() in (
                            "true",
                            "1",
                            "yes",
                        )
...
class EncryptedJSON(TypeDecorator):
    ...
    def process_bind_param(self, value: Any, dialect: Dialect) -> Optional[bytes]:
        ...
        try:
            ...
            return _get_key_management_service()._encrypt_data(
                json_str.encode("utf-8"), self._get_encryption_key()
            )
        except Exception as e:
    ...
    def process_result_value(self, value: Any, dialect: Dialect) -> Any:
        ...
        try:
            ...
            decrypted_bytes = _get_key_management_service()._decrypt_data(
                value, self._get_encryption_key()
            )
...
def encrypted_string(
    key_id: str, length: Optional[int] = None, **kwargs
) -> EncryptedString:
...
class EncryptionContext:
    ...
    def __exit__(self, exc_type, exc_val, exc_tb):
        ...
        encryption_metrics.record_operation(self.operation_name, duration_ms, success)

        if not success:
            logger.error(f"Encryption context {self.operation_name} failed: {exc_val}")

    def record_operation(
        self, operation: str, success: bool, duration_ms: Optional[int] = None
    ):
        """Record a sub-operation within the context"""
        if duration_ms is None:
            duration_ms = 0
        self.operations.append(
            {"operation": operation, "success": success, "duration_ms": duration_ms}
        )
