...
class KnowledgeManager:
    ...
    def get_statistics(self, agent_name: str) -> Dict[str, Any]:
        ...
        # Check for training status from RL Orchestrator
        last_consumption = None
        try:
            import requests
            response = requests.get(f"http://localhost:8011/agents/{agent_name}/training-status", timeout=1)
            if response.status_code == 200:
                training_data = response.json()
                if training_data.get("last_trained") and training_data["last_trained"] != "Never":
                    from datetime import datetime
                    last_consumption = datetime.fromisoformat(training_data["last_trained"])
        except:
            # If RL Orchestrator is not available, check legacy consumption tracking
            pass
        ...
        return {
            ...
            "new_knowledge_available": 0  # This could be calculated based on unconsumed items
        }
