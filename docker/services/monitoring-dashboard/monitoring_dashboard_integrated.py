...
class DashboardConfig:
    ...
    enhanced_api_url: str = "http://enhanced-api:8002"  # Enhanced API endpoint
...
class MonitoringDashboard:
    ...
    def _get_available_agents(self) -> List[str]:
        ...
        try:
            ...
            api_url = "http://enhanced-api:8002/api/v2/agents/list"
    ...
    def _render_task_assignment(self):
            ...
            if st.button("🔌 Connect to API"):
                try:
                    ...
                    if response.status_code == 200:
                        ...
                        try:
                            agents_response = requests.get("http://enhanced-api:8002/api/v2/agents/list")
                            if agents_response.status_code == 200:
                                agents_data = agents_response.json()
                                st.session_state.available_agents = [agent['id'] for agent in agents_data.get("agents", [])]
                        except:
                            pass
        ...
        if st.session_state.api_connected:
                ...
                with st.spinner("Processing task..."):
                    try:
                        ...
                        task_response = requests.post(
                            "http://enhanced-api:8002/api/v2/agents/execute",
                            json={
                        ...
                        if task_response.status_code == 200:
                            ...
                            try:
                                status_response = requests.get(
                                    f"http://enhanced-api:8002/api/v1/tasks/{task_id}",
                                    headers=headers,
                                    timeout=5
                                )
                    ...
                    with col2:
                        if st.button(f"Check", key=f"check_{pending_task['task_id']}"):
                            try:
                                ...
                                status_response = requests.get(
                                    f"http://enhanced-api:8002/api/v1/tasks/{pending_task['task_id']}",
                                    headers=headers,
                                    timeout=5
                                )
            ...
            if st.button("💬 Send", disabled=not chat_message):
                with st.spinner("Agent thinking..."):
                    try:
                        ...
                        response = requests.post(
                            f"http://enhanced-api:8002/chat/{chat_agent}",
                            json={
                        ...
                        if response.status_code == 200:
                            result = response.json()
                            st.markdown(f"**{chat_agent.title()}:** {result.get('agent_response', 'No response')}")
                        else:
            ...
            st.markdown("""
            **Requirements:**
            1. Make sure the Enhanced API is running on port 8002
            2. Run: `docker-compose up enhanced-api`
            """)
